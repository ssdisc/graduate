...
for idx = 1:size(pairs, 1)
    ...
    cfg = local_base_profile_cfg("narrowband", opts, 1, "fh_erasure", fullfile(suiteRoot, caseName));
    ...
    row = local_run_primary_case(cfg, "fh_erasure", opts, "narrowband", "coverage", caseName);
    row.paramA = center;
    row.paramB = bw;
...
for idx = 1:size(caseList, 1)
    ...
    cfg = local_base_profile_cfg("narrowband", opts, 3, "fh_erasure", fullfile(suiteRoot, caseName));
    cfg.channel.narrowband.centerFreqPoints = center;
    cfg.channel.narrowband.bandwidthFreqPoints = bw;
    local_validate_narrowband_case(cfg);
    row = local_run_primary_case(cfg, "fh_erasure", opts, "narrowband", "confidence", caseName);
    row.paramA = center;
    row.paramB = bw;
    row.paramAText = "centerFreqPoints";
    row.paramBText = "bandwidthFreqPoints";
...
for idx = 1:numel(centers)
    ...
    cfg = local_base_profile_cfg("narrowband", opts, 1, ["none" "fh_erasure"], fullfile(suiteRoot, caseName));
    cfg.channel.narrowband.centerFreqPoints = center;
    cfg.channel.narrowband.bandwidthFreqPoints = 1.0;
    local_validate_narrowband_case(cfg);
    row = local_run_compare_case(cfg, "none", "fh_erasure", opts, "narrowband", "research", caseName);
    row.paramA = center;
    row.paramB = 1.0;
    row.paramAText = "centerFreqPoints";
    row.paramBText = "bandwidthFreqPoints";
    row.minImprovement = 1e-12;
    row.pass = row.runOk && row.secondaryRawPer + 1e-12 < row.primaryRawPer;
    rows(end + 1, 1) = row; %#ok<AGROW>
end
