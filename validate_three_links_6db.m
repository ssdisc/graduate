...
for idx = 1:numel(centers)
    ...
    cfg = local_base_profile_cfg("narrowband", opts, 1, ["fh_erasure" "narrowband_subband_excision_soft"], fullfile(suiteRoot, caseName));
    cfg.channel.narrowband.centerFreqPoints = center;
    cfg.channel.narrowband.bandwidthFreqPoints = 1.0;
    local_validate_narrowband_case(cfg);
    row = local_run_compare_case(cfg, "fh_erasure", "narrowband_subband_excision_soft", opts, "narrowband", "research", caseName);
    row.paramA = center;
    row.paramB = 1.0;
    row.paramAText = "centerFreqPoints";
    row.paramBText = "bandwidthFreqPoints";
