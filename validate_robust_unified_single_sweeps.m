...
try
    ...
    fprintf("[RU-SINGLE] %-18s %-24s PER=%.4g rawPER=%.4g burst=%.3fs elapsed=%.3fs pass=%d\n", ...
        char(row.suite), char(row.caseName), row.per, row.rawPer, row.burstSec, row.elapsedSec, row.pass);
catch ME
