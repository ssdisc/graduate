function bits = decode_protected_header_symbols(rSym, infoBitsLen, frameCfg, fec, softCfg)
%DECODE_PROTECTED_HEADER_SYMBOLS  Decode short-header bits using PHY-header protection settings.

rSym = rSym(:);
infoBitsLen = max(0, round(double(infoBitsLen)));
...
bits = fit_bits_length(bits, infoBitsLen);
end
