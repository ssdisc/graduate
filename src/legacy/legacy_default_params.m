function p = legacy_default_params(opts)
...
p.linkBudget.ebN0dBList = 6:2:10;
...
p.frame.sessionHeaderBodyDiversity = struct();
p.frame.sessionHeaderBodyDiversity.enable = false;   % dedicated会话头正文跨频点分集：K 份完整正文拆到 K 个拉开间隔的 FH 频点
p.frame.sessionHeaderBodyDiversity.copies = 3;
p.frame.sessionHeaderBodyDiversity.freqSet = [];    % 空=从 p.fh.freqSet 自动挑选 K 个拉开间隔的频点
...
p.frame.phyHeaderSpreadFactor = 4;         % PHY小头DSSS扩频因子；提升对窄带中心频点命中的鲁棒性
...
p.frame.phyHeaderFhSymbolsPerHop = 124;    % 与默认compact头+spreadFactor=4匹配，避免单份头部跨频过快导致能量摊薄
...
p.fh.payloadDiversity = struct();
p.fh.payloadDiversity.enable = false;        % payload双频点分集：每个逻辑hop连续发两份，第二份落到远离频点
p.fh.payloadDiversity.copies = 2;
p.fh.payloadDiversity.indexOffset = 4;       % 默认8频点时取对侧频点，避免相邻命中
% 频率集合（归一化频率，相对于符号率）
...
p.mitigation.fhErasure.freqPowerRatioThreshold = 1.35; % 窄带命中后频点功率不一定尖锐，降低整频点门限
p.mitigation.fhErasure.hopPowerRatioThreshold = 1.55;  % 单跳补充检测更激进，避免坏hop漏判
p.mitigation.fhErasure.minReliability = 0.02;         % 被判定命中的符号软擦除可靠度
p.mitigation.fhErasure.softSlope = 12.0;              % 强窄带超过门限后快速压向擦除
p.mitigation.fhErasure.maxErasedFreqFraction = 0.75;  % 允许8频点里最多约6个频点被降权
...
p.mitigation.fhErasure.lowFreqPowerRatioThreshold = 0.75;
p.mitigation.fhErasure.lowHopPowerRatioThreshold = 0.70;
p.mitigation.fhErasure.lowPowerSoftSlope = 8.0;
p.mitigation.fhErasure.constellationMseEnable = true;
p.mitigation.fhErasure.constellationMseThreshold = 0.22;
p.mitigation.fhErasure.constellationMseSoftSlope = 8.0;
p.mitigation.fhErasure.freqConstellationMseEnable = true;   % 频点级星座失真聚合检测，补偿“能量不尖但误差很大”的窄带命中
p.mitigation.fhErasure.freqConstellationMseRatioThreshold = 1.05; % 频点MSE只要持续高于背景即视为可疑
p.mitigation.fhErasure.freqConstellationMseFloor = 0.03;    % 降低绝对门限，覆盖“能量中等但星座完全散开”的情况
p.mitigation.fhErasure.freqBandstopEnable = true;           % 先按频点聚合检测窄带并做频域抑制，再执行擦除降权
p.mitigation.fhErasure.freqBandstopObservationSymbols = 128; % 每个频点至少聚合到这么多符号再做可靠带阻检测
p.mitigation.fhErasure.freqBandstopPeakRatio = 2.2;         % 比普通fft_bandstop更敏感，适合FH频点聚合后的检测
p.mitigation.fhErasure.freqBandstopEdgeRatio = 1.2;
p.mitigation.fhErasure.freqBandstopMinFreqAbs = 0.005;      % 允许检测靠近DC的小频偏窄带
p.mitigation.fhErasure.freqBandstopMaxBandwidthFrac = 0.18; % 只压制明显窄于基带信号的窄带干扰
...
p.mitigation.binding.impulseMethods = ["none" "blanking" "clipping"];
p.mitigation.binding.singleToneMethods = ["none" "fft_notch" "adaptive_notch" "adaptive_ml_frontend"];
p.mitigation.binding.narrowbandMethods = ["none" "fh_erasure"];
...
p.rxSync.multipathEq.enable = false;
...
p = legacy_apply_link_profile(p, opts.linkProfileName);
...
if any(requestedMlModels == "cnn") && requiredMlModels.cnn
...
if any(requestedMlModels == "gru") && requiredMlModels.gru
...
if any(requestedMlModels == "selector") && requiredMlModels.selector
...
if any(requestedMlModels == "narrowband") && requiredMlModels.narrowband
...
if any(requestedMlModels == "fh_erasure") && requiredMlModels.fhErasure
...
if any(requestedMlModels == "multipath_eq") && requiredMlModels.multipathEq
...
function required = local_required_ml_models_from_config(p)
[methodsUse, ~, ~] = resolve_mitigation_methods(p.mitigation, p.channel);
methodsUse = lower(string(methodsUse(:).'));
required = struct();
required.lr = any(methodsUse == "ml_blanking");
required.cnn = any(methodsUse == "ml_cnn" | methodsUse == "ml_cnn_hard");
required.gru = any(methodsUse == "ml_gru" | methodsUse == "ml_gru_hard" | methodsUse == "adaptive_ml_frontend");
required.selector = any(methodsUse == "adaptive_ml_frontend");
required.narrowband = any(methodsUse == "ml_narrowband" | methodsUse == "adaptive_ml_frontend");
required.fhErasure = any(methodsUse == "ml_fh_erasure");
required.multipathEq = local_multipath_eq_offline_requested_local(p);
end

function tf = local_multipath_eq_offline_requested_local(p)
tf = false;
if ~(isfield(p, "rxSync") && isstruct(p.rxSync) ...
        && isfield(p.rxSync, "multipathEq") && isstruct(p.rxSync.multipathEq))
    error("p.rxSync.multipathEq is required.");
end
