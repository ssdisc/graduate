...
switch profileName
    ...
    case "narrowband"
        ...
        profileRxCfg.allowedMethods = ["none" "fh_erasure" "ml_fh_erasure" "narrowband_notch_soft" ...
    ...
    case "robust_unified"
        ...
        commonTx.waveform.rolloff = 0.25;
        commonTx.waveform.spanSymbols = 6;
        commonTx.payload.tileJp2.interleaveMode = "polyphase";
        profileTxCfg.fh.enable = true;
        profileTxCfg.fh.nFreqs = 8;
        ...
        robustControlFreqSet = controlPlan.controlOnlyFreqSet([1 ceil(numel(controlPlan.controlOnlyFreqSet) / 2) numel(controlPlan.controlOnlyFreqSet)]);
        commonTx.control.sessionHeaderMode = "session_frame_repeat";
        commonTx.control.sessionFrameRepeatCount = 3;
        commonTx.control.preambleLength = 127;
        commonTx.control.packetSyncLength = 63;
        commonTx.control.resyncIntervalPackets = 1;
        commonTx.control.phyHeaderFhEnable = true;
        commonTx.control.phyHeaderRepeatCompact = 4;
        commonTx.control.phyHeaderSpreadFactor = 6;
        commonTx.control.preambleDiversity.enable = true;
        commonTx.control.preambleDiversity.copies = numel(robustControlFreqSet);
        commonTx.control.preambleDiversity.freqSet = robustControlFreqSet;
        commonTx.control.sessionHeaderBodyDiversity.enable = true;
        commonTx.control.sessionHeaderBodyDiversity.copies = numel(robustControlFreqSet);
        commonTx.control.sessionHeaderBodyDiversity.freqSet = robustControlFreqSet;
        commonTx.control.phyHeaderDiversity.enable = true;
        ...
        commonTx.innerCode.ldpc.softBits = 6;
        commonTx.innerCode.ldpc.maxIterations = 10;
        commonTx.interleaver.nRows = 128;
...
mitigation.binding = struct( ...
    ...
    "narrowbandMethods", ["none" "fh_erasure" "ml_fh_erasure" "narrowband_notch_soft" ...
        "narrowband_subband_excision_soft" "narrowband_cnn_residual_soft"], ...
...
mitigation.robustMixed = struct( ...
    ...
    "enableFhReliabilityFloorWithMultipath", false, ...
    "fhReliabilityFloorWithMultipath", 0.70, ...
    "minReliability", 0.05, ...
    "sampleNbiCancel", struct( ...
        "peakRatio", 2.6, ...
        "edgeRatio", 1.25, ...
        "maxBands", 1, ...
        "mergeGapBins", 3, ...
        ...
        "smoothSpanBins", 9, ...
        "fftOversample", 2, ...
        "maxBandwidthFrac", 0.38, ...
        "minFreqAbs", 0.0, ...
        "suppressToFloor", true), ...
    "scFdeNbiCancel", struct( ...
        "peakRatio", 3.4, ...
        "edgeRatio", 1.45, ...
        "maxBands", 1, ...
        "mergeGapBins", 2, ...
        ...
        "smoothSpanBins", 5, ...
        "fftOversample", 4, ...
        "maxBandwidthFrac", 0.16, ...
        "minFreqAbs", 0.005, ...
        "suppressToFloor", true, ...
        "maxMaskFraction", 0.22, ...
        "reliabilityPenaltySlope", 0.45));
end
...
switch profileName
    ...
    case "robust_unified"
        mapperName = "robust_joint_stfd_mapper";
    otherwise
        error("Unexpected profile name: %s", char(profileName));
end
end
...
switch profileName
    ...
    case "robust_unified"
        chainName = "run_robust_unified_rx";
    otherwise
        error("Unexpected profile name: %s", char(profileName));
end
end
