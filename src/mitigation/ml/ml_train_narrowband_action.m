function [model, report] = ml_train_narrowband_action(p, opts)
%ML_TRAIN_NARROWBAND_ACTION  Train a binary MLP that gates FFT bandstop for narrowband interference.

arguments
    p (1,1) struct
    opts.nBlocks (1,1) double {mustBePositive} = 1200
    opts.ebN0dBRange (1,2) double = [-2 14]
    opts.blockLenRange (1,2) double {mustBePositive} = [96 1024]
    opts.epochs (1,1) double {mustBeInteger, mustBePositive} = 50
...
if opts.useGpu && canUseGPU()
    ...
    if opts.verbose
        fprintf("使用GPU训练窄带动作模型\n");
    end
else
    executionEnvironment = "cpu";
    if opts.verbose
        fprintf("使用CPU训练窄带动作模型\n");
    end
end

dataset = ml_generate_narrowband_dataset(p, opts.nBlocks, opts.ebN0dBRange, ...
    "blockLenRange", opts.blockLenRange, ...
    "maxRetriesPerBlock", opts.maxRetriesPerBlock, ...
    "bpskProbability", opts.bpskProbability, ...
...
if split.nVal < 1 || split.nTest < 1
    error("当前窄带动作训练流程要求独立的验证集和测试集，请增大 nBlocks 或调整 val/test 占比。");
end
...
for epoch = 1:opts.epochs
    ...
    for bStart = 1:opts.batchSize:size(XTrain, 1)
        ...
        [loss, gradients] = dlfeval(@local_narrowband_loss, model.net, XBatch, yBatch, classWeights, nClasses);
    ...
    valLoss = local_eval_narrowband_loss(model.net, XVal, yVal, classWeights, nClasses, executionEnvironment);
    ...
    if opts.verbose && (epoch == 1 || mod(epoch, 5) == 0 || epoch == opts.epochs)
        valMetricsNow = local_narrowband_metrics(model.net, XVal, yVal, classNames);
...
valMetrics = local_narrowband_metrics(model.net, XVal, yVal, classNames);
testMetrics = local_narrowband_metrics(model.net, XTest, yTest, classNames);

report = struct();
report.nBlocks = dataset.nBlocks;
report.ebN0dBRange = dataset.ebN0dBRange;
report.blockLenRange = opts.blockLenRange;
report.rngSeed = rngSeed;
report.trainingOptions = opts;
report.trainingContext = ml_capture_training_context(p);
report.reloadContext = ml_capture_narrowband_reload_context(p);
...
if opts.saveArtifacts
    [report, ~] = ml_save_training_artifacts(model, report, "narrowband_action_model", ...
...
function [loss, gradients] = local_narrowband_loss(net, XBatch, yBatch, classWeights, nClasses)
...
function loss = local_eval_narrowband_loss(net, XVal, yVal, classWeights, nClasses, executionEnvironment)
if isempty(XVal)
    loss = NaN;
    return;
end
...
function metrics = local_narrowband_metrics(net, X, y, classNames)
