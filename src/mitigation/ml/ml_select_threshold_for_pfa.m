...
bestIdx = local_select_best_index(searchIdx, string(opts.policy), pdArr, pfaArr, peArr, candidates);
...
function bestIdx = local_select_best_index(searchIdx, policy, pdArr, pfaArr, peArr, candidates)
searchIdx = searchIdx(:);
switch lower(policy)
    case "min_pe_under_pfa"
        rank = [peArr(searchIdx), pfaArr(searchIdx), -pdArr(searchIdx), candidates(searchIdx)];
    case "max_pd_under_pfa"
        rank = [-pdArr(searchIdx), pfaArr(searchIdx), peArr(searchIdx), candidates(searchIdx)];
    case "min_pe"
        rank = [peArr(searchIdx), pfaArr(searchIdx), -pdArr(searchIdx), candidates(searchIdx)];
    otherwise
