...
sampler.narrowbandBandwidthFreqPointsRange = local_resolve_range(double(p.channel.narrowband.bandwidthFreqPoints), opts.narrowbandBandwidthFreqPointsRange, ...
    "narrowbandBandwidthFreqPointsRange", 1e-6, inf);
[maxCenterFreqPoints, ~] = narrowband_center_freq_points_limit(p.fh, resolve_waveform_cfg(p), sampler.narrowbandBandwidthFreqPointsRange(2));
sampler.narrowbandCenterFreqPointsRange = local_resolve_range(double(p.channel.narrowband.centerFreqPoints), opts.narrowbandCenterFreqPointsRange, ...
    "narrowbandCenterFreqPointsRange", -maxCenterFreqPoints, maxCenterFreqPoints);
...
sampler.summary = struct( ...
    ...
    "narrowbandCenterFreqPointsRange", sampler.narrowbandCenterFreqPointsRange, ...
    "narrowbandBandwidthFreqPointsRange", sampler.narrowbandBandwidthFreqPointsRange, ...
...
profile = struct( ...
    ...
    "narrowbandPower", NaN, ...
    "narrowbandCenterFreqPoints", NaN, ...
    "narrowbandBandwidthFreqPoints", NaN, ...
    "sweepEnable", false, ...
    "sweepPower", NaN, ...
    "sweepStartHz", NaN, ...
    "sweepStopHz", NaN, ...
...
if extraFlags(2)
    ...
    pBlock.channel.narrowband.centerFreqPoints = local_sample_uniform(sampler.narrowbandCenterFreqPointsRange);
    pBlock.channel.narrowband.bandwidthFreqPoints = local_sample_uniform(sampler.narrowbandBandwidthFreqPointsRange);
    profile.narrowbandEnable = true;
    profile.narrowbandPower = pBlock.channel.narrowband.power;
    profile.narrowbandCenterFreqPoints = pBlock.channel.narrowband.centerFreqPoints;
    profile.narrowbandBandwidthFreqPoints = pBlock.channel.narrowband.bandwidthFreqPoints;
end
...
profileSet = struct( ...
    ...
    "narrowbandCenterFreqPoints", nan(nBlocks, 1), ...
    "narrowbandBandwidthFreqPoints", nan(nBlocks, 1), ...
...
summary.realizedNarrowbandCenterFreqPointsRange = local_realized_range(profile.narrowbandCenterFreqPoints, profile.narrowbandEnable);
summary.realizedNarrowbandBandwidthFreqPointsRange = local_realized_range(profile.narrowbandBandwidthFreqPoints, profile.narrowbandEnable);
