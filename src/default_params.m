...
p.outerRs.parityPacketsPerBlock = 6;   % 每个RS块附加的校验包数 P
...
p.frame.phyHeaderFhSymbolsPerHop = 4;      % slow模式下的头部每跳符号数；头部分集开启时自动改为“一份完整头部一跳”
...
p.mitigation.adaptiveFrontend.narrowbandGuard.overrideClasses = ["clean" "multipath"];
p.mitigation.adaptiveFrontend.narrowbandGuard.probePeakRatio = 4.0;
...
p.mitigation.adaptiveFrontend.narrowbandGuard.minFftPeakRatio = 6.0;
...
p.mitigation.binding.multipathMethods = ["none" "fh_erasure" "ml_fh_erasure"];
...
p.rxSync.multipathEq.method = "mmse";      % "mmse" | "zf" | "ml_ridge" | "ml_mlp"，均为跳频感知；ml_mlp为离线ML符号均衡
...
p.rxSync.multipathEq.compareMethods = ["none" "mmse" "zf" "ml_ridge"];
p.rxSync.multipathEq.mitigationCompareEqualizers = "mmse"; % 纯多径下fh_erasure/ml_fh_erasure只挂到这些均衡器
