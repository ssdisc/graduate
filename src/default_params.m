...
p.channel.singleTone.power = 1.25;      % 单音固定接收功率（线性值）
...
p.channel.narrowband.power = 1.0;       % 窄带噪声固定接收功率（线性值）
...
p.channel.sweep.power = 1.0;            % 扫频干扰固定接收功率（线性值）
...
p.covert.warden.nTrials = 200; % 蒙特卡洛试验次数
