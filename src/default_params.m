...
p.rxSync.multipathEq.method = "mmse";      % "mmse" | "zf"，均为跳频感知FFE
p.rxSync.multipathEq.nTaps = 9;            % 线性FFE长度（符号数）
p.rxSync.multipathEq.lambdaFactor = 1.0;   % MMSE正则: lambda=lambdaFactor*N0
p.rxSync.multipathEq.compareEnable = true;     % 多径启用时直接在主链路展开 none/mmse/zf 接收分支
p.rxSync.multipathEq.compareMethods = ["none" "mmse" "zf"];
