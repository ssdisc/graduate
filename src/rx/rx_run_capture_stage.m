...
front = capture_synced_block_with_diversity( ...
    ctx.rxCapture, ctx.pkt.syncSym(:), ctx.expectedLen, ctx.syncCfg, ctx.runtimeCfg.mitigation, ...
    ctx.runtimeCfg.mod, ctx.waveform, ctx.sampleAction, local_capture_bootstrap_chain_local(ctx.method), ...
    ctx.fhCaptureCfg, ctx.runtimeCfg.rxDiversity);
