...
%                 .keepCols  - 保留低频列数（默认4）
%                 .quantStep - 量化步长（默认16）
%             .tileJp2      - 分块JPEG 2000配置（codec='tile_jp2'时）
...
switch codec
    ...
    case "dct"
        ...
        codecMeta = struct( ...
            "blockSize", double(codecCfg.dct.blockSize), ...
            "keepRows", double(codecCfg.dct.keepRows), ...
            "keepCols", double(codecCfg.dct.keepCols), ...
            "quantStep", double(codecCfg.dct.quantStep));
    case "tile_jp2"
        [bytes, codecMeta] = tile_jp2_encode_bytes(uint8(img), codecCfg.tileJp2);
    case "toolbox_image"
