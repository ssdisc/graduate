...
local_require_fields(channel.singleTone, ["enable" "weight" "randomPhase"], structName + ".singleTone");
local_require_fields(channel.narrowband, ["enable" "weight" "centerFreqPoints" "bandwidthFreqPoints"], structName + ".narrowband");
local_require_fields(channel.sweep, ["enable" "weight" "periodSymbols" "randomPhase"], structName + ".sweep");
...
channelCfg = struct( ...
    ...
    "singleToneFreqHz", local_require_alias_finite_scalar(channel.singleTone, ["freqHz" "normFreq"], structName + ".singleTone"), ...
    ...
    "sweepStartHz", local_require_alias_finite_scalar(channel.sweep, ["startHz" "startFreq"], structName + ".sweep"), ...
    "sweepStopHz", local_require_alias_finite_scalar(channel.sweep, ["stopHz" "stopFreq"], structName + ".sweep"), ...
...
function value = local_require_alias_finite_scalar(s, fieldNames, structName)
fieldNames = string(fieldNames(:).');
for idx = 1:numel(fieldNames)
    fieldName = fieldNames(idx);
    if isfield(s, fieldName)
        value = local_require_finite_scalar(s.(fieldName), structName + "." + fieldName);
        return;
    end
end
error("export_thesis_tables:MissingField", ...
    "Missing one of required fields %s in %s.", strjoin(cellstr(fieldNames), "/"), structName);
end
