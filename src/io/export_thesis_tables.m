...
local_require_fields(results, ["methods" "ebN0dB" "ber" "packetDiagnostics" "linkBudget" "spectrum" "tx" "params"]);
...
runSummary = local_build_run_summary_table(results, methods, nMethods, nPoints);
pointOverview = local_build_point_overview_table( ...
    bobEbN0dB, bobBudget, frontEndBob, headerBob, results, nPoints);
metricsBob = local_build_metric_table( ...
    methods, "bob", bobBudget.linkGainDb, bobEbN0dB, berBob, payloadBob, ...
...
if isfield(results, "eve")
    local_require_fields(results.eve, ["ebN0dB" "ber" "packetDiagnostics"], "results.eve");
    local_require_fields(results.packetDiagnostics, "bob", "results.packetDiagnostics");
    ...
    metricsEve = local_build_metric_table( ...
        methods, "eve", eveBudget.linkGainDb, eveEbN0dB, berEve, payloadEve, ...
...
if isfield(results, "covert") && isfield(results.covert, "warden")
    wardenTable = local_build_warden_table(results.covert.warden, bobEbN0dB, nPoints);
    writetable(wardenTable, fullfile(outDir, "warden_layers.csv"));
end
end

function t = local_build_run_summary_table(results, methods, nMethods, nPoints)
...
t = table( ...
    ...
    nMethods, ...
    nPoints, ...
    double(results.linkBudget.txPowerLin), ...
    double(results.linkBudget.txPowerDb), ...
    double(results.linkBudget.noisePsdLin), ...
    local_join_numeric_vector(results.linkBudget.bob.linkGainDb), ...
    double(results.tx.burstDurationSec), ...
    double(results.tx.averagePowerLin), ...
    double(results.tx.maxBurstDurationSec), ...
    double(results.tx.maxAveragePowerLin), ...
    ...
    'VariableNames', { ...
        ...
        'fhEnable', 'packetConcealActive', 'packetConcealMode', 'methods', 'nMethods', 'nPoints', ...
        'txPowerLin', 'txPowerDb', 'noisePsdLin', 'bobLinkGainDbList', ...
        'txBurstDurationSec', 'txAveragePowerLin', 'txMaxBurstDurationSec', 'txMaxAveragePowerLin', ...
        'bw99Hz', 'etaBpsHz', 'eveEnable', 'wardenEnable', ...
        'eveScrambleAssumption', 'eveFhAssumption', 'eveChaosAssumption', 'eveChaosApproxDelta', ...
        'wardenPrimaryLayer'});
end

function t = local_build_point_overview_table(bobEbN0dB, bobBudget, frontEndBob, headerBob, results, nPoints)
...
wardenLinkGainDb = nan(nPoints, 1);
...
t = table( ...
    ...
    wardenEbN0dB, ...
    'VariableNames', { ...
        'pointIndex', 'bobLinkGainDb', 'bobEbN0dB', ...
        'bobFrontEndSuccessRate', 'bobHeaderSuccessRate', ...
        'klSignalVsNoise', 'klNoiseVsSignal', 'klSymmetric', ...
        'eveLinkGainDb', 'eveEbN0dB', 'eveFrontEndSuccessRate', 'eveHeaderSuccessRate', ...
        'wardenLinkGainDb', 'wardenEbN0dB'});
end

function t = local_build_metric_table(methods, roleName, linkGainDb, ebN0dB, ber, payloadSuccessRate, commMetrics, compMetrics, frontEndSuccessRate, headerSuccessRate)
nMethods = numel(methods);
...
t = table( ...
    ...
    pointIndex, ...
    repmat(double(linkGainDb(:)), nMethods, 1), ...
    ...
    compSsimVec, ...
    'VariableNames', { ...
        'role', 'methodIndex', 'method', 'pointIndex', 'linkGainDb', 'ebN0dB', ...
        'frontEndSuccessRate', 'headerSuccessRate', 'ber', 'payloadSuccessRate', ...
        'mseComm', 'psnrComm', 'ssimComm', 'mseComp', 'psnrComp', 'ssimComp'});
end

function t = local_build_warden_table(wardenResults, bobEbN0dB, nPoints)
...
for i = 1:numel(layerNames)
    ...
    rows = [rows; table( ... %#ok<AGROW>
        ...
        pd, pfa, pmd, pe, xi, ...
        'VariableNames', { ...
            'layer', 'referenceLink', 'pointIndex', 'bobEbN0dB', 'wardenEbN0dB', ...
            'pd', 'pfa', 'pmd', 'pe', 'xi'})];
end
...
local_require_fields(budgetIn, ["linkGainDb" "ebN0dB"], structName);
budget = struct();
budget.linkGainDb = local_require_numeric_vector(budgetIn.linkGainDb, nPoints, structName + ".linkGainDb");
