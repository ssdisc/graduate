...
| 功能 | 文件 | 说明 |
|------|------|------|
| 前导生成 | `make_preamble.m` | 支持PN/混沌前导，用于帧同步 |
| 帧头构建 | `build_header_bits.m` | Magic字(0xA55A) + 图像尺寸 + 载荷长度 |
| 帧同步 | `frame_sync.m` | 基于前导相关的粗同步（支持混沌前导） |
| 定时跟踪 | `timing_dll_sync.m` | Early-Late门DLL维持符号定时对齐 |
...
| 功能 | 文件 | 说明 |
|------|------|------|
| BPSK/QPSK/MSK调制 | `modulate_bits.m` | BPSK: 0→+1,1→-1；QPSK: Gray映射；MSK: 连续相位h=0.5 |
...
|------|------|-------------|
| `none` | 不处理 | 否 |
| `fft_notch` | FFT变换域峰值检测+陷波 | 否 |
| `adaptive_notch` | 自适应IIR陷波（按频谱峰估计） | 否 |
| `blanking` | 幅度阈值置零 | 否 |
| `clipping` | 幅度阈值削波 | 否 |
...
### 抑制方法
```matlab
p.mitigation.methods = ["none" "fft_notch" "adaptive_notch" "blanking" "ml_cnn" "ml_gru"];
...
% p.mod.type = 'MSK';  % 连续相位h=0.5，差分相位软解调
```
